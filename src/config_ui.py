...
class ConfigWindow(QWidget):
    ...
    def init_ui(self):
        ...
        # OAuth Setup Button
        self.oauth_setup_btn = QPushButton("Setup Google OAuth")
        self.oauth_setup_btn.clicked.connect(self.setup_oauth)
        form_layout.addRow(self.oauth_setup_btn)
        
        # Connect Button (only enabled if OAuth is configured)
    ...
    def setup_oauth(self):
        """Guide user through Google OAuth setup with instructions."""
        from PySide6.QtWidgets import QDialog, QVBoxLayout, QTextEdit, QPushButton, QLabel, QHBoxLayout, QMessageBox, QFileDialog
        ...
        # Instructions
        instructions = QLabel("To connect your Google Calendar, you need to set up OAuth credentials:")
...
4. Create OAuth 2.0 credentials:
   ...
   - Give it a name (e.g., "GlowStatus")
   - Click "Create"
5. Download the JSON file (it will be named something like "client_secret_xxx.json")
6. Use the button below to select that downloaded JSON file

Note: You may need to add your own email to the test users list if your app is in testing mode.
"""
        
        steps_label = QLabel(steps_text)
        steps_label.setWordWrap(True)
        steps_label.setStyleSheet("background-color: #f0f0f0; padding: 10px; margin: 10px 0;")
        layout.addWidget(steps_label)
        
        # File selection button
        file_btn = QPushButton("Select client_secret.json file")
        file_btn.clicked.connect(lambda: self.select_oauth_file(dialog))
        layout.addWidget(file_btn)
        
        # Alternative: paste JSON content
        alt_label = QLabel("Alternative: Paste the JSON content directly:")
        layout.addWidget(alt_label)
        
        self.json_text = QTextEdit()
        self.json_text.setPlaceholderText("Paste the contents of your client_secret.json file here...")
        self.json_text.setMaximumHeight(150)
        layout.addWidget(self.json_text)
        
        # Buttons
        btn_layout = QHBoxLayout()
        save_btn = QPushButton("Save OAuth Config")
        save_btn.clicked.connect(lambda: self.save_oauth_config(dialog))
    ...
    def select_oauth_file(self, dialog):
        """Let user select the client_secret.json file."""
        file_path, _ = QFileDialog.getOpenFileName(
            dialog,
            "Select Google OAuth Client Secret File",
            "",
        ...
        if file_path:
            try:
                ...
                QMessageBox.warning(dialog, "Error", f"Could not read file: {e}")
    
    def save_oauth_config(self, dialog):
        """Save the OAuth configuration."""
        json_content = self.json_text.toPlainText().strip()
        
        if not json_content:
            QMessageBox.warning(dialog, "Error", "Please select a file or paste JSON content.")
            return
        ...
        try:
            ...
            # Save the client_secret.json file
            client_secret_path = resource_path('resources/client_secret.json')
            ...
            QMessageBox.information(dialog, "Success", "OAuth configuration saved successfully!\n\nYou can now click 'Connect Google Account' to authenticate.")
            dialog.accept()
            
        except json.JSONDecodeError:
            QMessageBox.warning(dialog, "Error", "Invalid JSON content. Please check your client_secret.json file.")
        except Exception as e:
            QMessageBox.warning(dialog, "Error", f"Could not save OAuth configuration: {e}")